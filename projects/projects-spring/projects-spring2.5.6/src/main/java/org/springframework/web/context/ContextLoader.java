...
public class ContextLoader {
	 ...
	 */
	//contextConfigLocation是在web.xml中配置的是指定applicationContext.xml的位置的
	 ...
	 */
	//ConfigurableWebApplicationContext是context真实的的类型，这个类型是WebApplicatinContext
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext)
		...
		try {
			...
			//其中key为Thread.currentThread().getContextClassLoader()即当前线程类加载器  
	...
	protected WebApplicationContext createWebApplicationContext(
		...
		//得到的类是XmlWebApplicationContext这个类
		...
		//这个setConfigLocation()这个方法在父类AbstractRefreshableConfigApplicationContext中出现的
		//将在web.xml中配置的applicationContext.xml文件的路径设置到XmlWebApplicationContext中的属性中
		//如果没有设置这个参数，那么就会传递null，在调用配置文件的时候会使用XmlWebApplicationContext类的
		//getDefaultConfigLocations();方法读取默认web-info下面的配置文件
	...
	protected Class determineContextClass(ServletContext servletContext) throws ApplicationContextException {
		//得到servletContext中的contextclassname，如果servletContext中存在这个contextClassName,那么就用这个contextClassName
		//指定要生成的ApplicationContext
		...
		if (contextClassName != null) {
			...
			//如果在servletContext中没有这个contextClassName中的元素，那么就开始读取中ContextLoader.properties这个配置
			//文件中org.springframework.web.context.WebApplicationContext=org.springframework.web.context.support.XmlWebApplicationContext
			//这个最终生成的类是XmlWebApplicationContextz这个类，而这个类也间接的继承了refresh()这个方法，可以解决为什么找不到refresh()方法
